...
module varuna::call_sui_usdc_0940 {
    ...
    public struct CALL_SUI_USDC_0940 has drop {}

    // Constants for this option
    const STRIKE_PRICE: u64 = 940_000_000; // 2000 USDC (with 9 decimals precision)
    ...
    fun init(witness: CALL_SUI_USDC_0940, ctx: &mut TxContext) {
        // Create the currency with the One-Time Witness
        let (treasury_cap, metadata) = coin::create_currency(
            witness,
            9,                                      // decimals (9 for Sui standard)
            b"CALL-SUI-USDC-0940",                 // symbol
            b"CALL Option SUI/USDC Strike 0940",   // name
            b"Decentralized CALL option with strike 0940 USDC per SUI", // description
    ...
    public fun create_pool<SUI, USDC>(
        treasury_cap: TreasuryCap<CALL_SUI_USDC_0940>,
        deepbook_pool_id: ID,
        clock: &Clock,
        ctx: &mut TxContext,
    ): ID {
        options_pool::create_pool<CALL_SUI_USDC_0940, SUI, USDC>(
            treasury_cap,
            0, // OPTION_TYPE_CALL
            STRIKE_PRICE,
            EXPIRATION_DATE,
