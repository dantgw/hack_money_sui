...
function App() {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    cn(
      "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-bold uppercase tracking-wide transition-colors",
        ...
        <div className="w-full flex h-12 sm:h-14 items-center justify-between px-3 sm:px-4 gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0 min-w-0">
            <Button
              ...
              className="font-display text-xs sm:text-sm h-8 sm:h-9 px-2 sm:px-3 shrink-0 normal-case"
            >
              {currentNetwork === "mainnet" ? "Mainnet" : "Testnet"}
            </Button>
            <div className="hidden lg:block [&_button]:h-8 [&_button]:text-xs sm:[&_button]:h-9 sm:[&_button]:text-sm [&_button]:font-display [&_button]:rounded-lg [&_button]:border [&_button]:border-border [&_button]:shadow-clay-sm [&_button]:normal-case">
              <ConnectButton />
            </div>
          </div>
        </div>
          ...
          <NavLink to="/" className={bottomTabClass}>
            {({ isActive }) => (
              <>
                ...
                <span className="text-[11px] font-bold uppercase">Trade</span>
              </>
            )}
          </NavLink>
          <NavLink to="/options" className={bottomTabClass}>
            {({ isActive }) => (
              <>
                ...
                <span className="text-[11px] font-bold uppercase">Options</span>
              </>
            )}
          </NavLink>
          <NavLink to="/account" className={bottomTabClass}>
            {({ isActive }) => (
              <>
                ...
                <span className="text-[11px] font-bold uppercase">Account</span>
              </>
            )}
          </NavLink>
        </div>
