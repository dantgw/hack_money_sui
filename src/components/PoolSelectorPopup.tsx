...
export function PoolSelectorPopup({ isOpen, onClose, pools, onSelect, selectedPoolName }: PoolSelectorPopupProps) {
    ...
    return (
        <div
            ref={dropdownRef}
            className="absolute top-full left-0 mt-1 z-50 w-[calc(100vw-16px)] max-w-[700px] sm:w-[700px] bg-card border-2 border-border rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[70vh] sm:max-h-[60vh]"
        >
            {/* Search Header */}
            <div className="p-4 border-b-2 border-border space-y-4">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-background border-2 border-border rounded-lg py-2.5 pl-10 pr-4 text-sm focus:ring-1 focus:ring-primary outline-none"
                        value={search}
                        onChange={e => setSearch(e.target.value)}
                    />
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center space-x-1.5 pointer-events-none">
                        <span className="text-[10px] bg-muted/30 px-1.5 py-0.5 rounded border-2 border-border text-muted-foreground">⌘K</span>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-1 text-xs font-bold text-muted-foreground uppercase tracking-tighter">
                    {['All', 'Spot'].map(cat => (
                        <button
                            ...
                            className={`px-3 py-2 sm:py-1.5 rounded-md transition-colors min-h-[44px] sm:min-h-0 touch-manipulation ${category === cat ? 'bg-primary/10 text-primary' : 'hover:text-foreground active:bg-muted/50'}`}
                        >
                            {cat}
                        </button>
                    ))}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                <table className="w-full text-left text-[13px]">
                    <thead className="sticky top-0 bg-card text-muted-foreground font-bold border-b-2 border-border uppercase text-[10px] tracking-widest">
                        <tr>
                            ...
                            return (
                                ...
                                >
                                    <td className="pl-4 pr-4 sm:pr-2 py-3">
                                        <Star className={`w-3.5 h-3.5 transition-colors ${isSelected ? 'text-yellow-500 fill-yellow-500' : 'text-muted-foreground group-hover:text-foreground/60'}`} />
                                    </td>
                                    ...
                                    <td className={`py-3 text-right font-medium ${isPositive ? 'text-primary' : 'text-destructive'}`}>
            ...
            {/* Footer */}
            <div className="p-2 bg-background border-t-2 border-border flex items-center justify-between text-[10px] text-muted-foreground font-medium uppercase">
